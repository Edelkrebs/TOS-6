...
uint8_t init_gpt(){
    ...
    volatile uint16_t* gpt_partition_entries_raw = (volatile uint16_t*)kmalloc(0x4000);

    ahci_read(primary_sata_device, gpt_header->guid_entry_lba, 32, gpt_partition_entries_raw);

    gpt_partition_entries = (uint8_t*)gpt_partition_entries_raw;
