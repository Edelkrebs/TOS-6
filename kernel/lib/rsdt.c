...
void* find_sdt_entry(char* signature){
	if(sdt_type == eXSDT){
		...
		for(uint64_t i = 0; i < sdt_entry_count; i++){
			ACPISDTheader* curr_header = (ACPISDTheader*)(curr_sdt->pointer_sdts[i] + VM_OFFSET);
		...
		for(uint64_t i = 0; i < sdt_entry_count; i++){
			ACPISDTheader* curr_header = (ACPISDTheader*)((uint64_t)curr_sdt->pointer_sdts[i] + VM_OFFSET);
			if(memcmp((uint8_t*)(curr_header->signature), (uint8_t*)signature, 4)  == 0){
				return (void*)curr_header;
			}
		}
